...
// End of UMD module
	...
	// Close all selects except the one passed
	...
	// Toggle open/closed states for non-touch
	...
	// Update styled selected options and real select box
	var changeRealSelectBox = function(realSelect, styledSelect, newValue, newLabel) {
	...
	return function(selector) {
		...
		var realSelects = queryAll(selector);

		if ( ! realSelects.length ) {
			return;
		}
		...
		realSelects.forEach(function (realSelect) {
			...
			styleSelectOptions.forEach(function(unused, index){
				...
				styleSelectOption.addEventListener('click', function(ev) {
					var target = ev.target,
						uuid = styledSelect.getAttribute('data-ss-uuid'),
		...
		query('html').addEventListener('click', function(ev){
			if ( ! isAncestorOf(ev.target, '.style-select', true) ) {
				closeAllStyleSelects();
			}
		});

		return realSelects.length > 1 ? realSelects : realSelects[0];
